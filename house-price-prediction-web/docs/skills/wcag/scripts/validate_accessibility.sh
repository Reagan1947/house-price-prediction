...
# Prerequisites:
#   - Node.js >= 18
#   - pa11y (installed automatically if missing)
#
# Exit codes:
#   0 = no issues found
...
# Shared temporary paths cleaned at exit
PA11Y_TEMP_DIR=""
JSON_PARTS_DIR=""
cleanup() {
    if [[ -n "$JSON_PARTS_DIR" && -d "$JSON_PARTS_DIR" ]]; then
        rm -rf "$JSON_PARTS_DIR"
    fi
    if [[ -n "$PA11Y_TEMP_DIR" && -d "$PA11Y_TEMP_DIR" ]]; then
        rm -rf "$PA11Y_TEMP_DIR"
    fi
}
trap cleanup EXIT
...
elif [[ -x "./node_modules/.bin/pa11y" ]]; then
    PA11Y_BIN="./node_modules/.bin/pa11y"
else
    echo "pa11y not found. Installing a temporary local copy..."
    PA11Y_TEMP_DIR="$(mktemp -d)"
    if ! npm install --prefix "$PA11Y_TEMP_DIR" --no-audit --no-fund --no-package-lock pa11y; then
        echo "Error: Failed to install pa11y. Please check your npm registry/network settings." >&2
        exit 2
    fi
    PA11Y_BIN="$PA11Y_TEMP_DIR/node_modules/.bin/pa11y"
fi

if [[ ! -x "$PA11Y_BIN" ]]; then
    echo "Error: pa11y executable not found: $PA11Y_BIN" >&2
    exit 2
fi
...
for url in "${TARGETS[@]}"; do
    ...
    set +e
    RESULT=$("$PA11Y_BIN" --standard "$STANDARD" --reporter cli "$url" 2>&1)
    EXIT_CODE=$?
    set -e
    ...
    if [[ -n "$JSON_PARTS_DIR" ]]; then
        set +e
        "$PA11Y_BIN" --standard "$STANDARD" --reporter json "$url" > "$JSON_PARTS_DIR/part_${PAGE_INDEX}.json" 2>/dev/null
        set -e
    fi
